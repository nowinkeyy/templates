...
![Outline Screenshot](https://raw.githubusercontent.com/labring-sigs/templates/main/template/outline/website-screenshot.webp)
...
- `oidc_username_claim` defaults to `sub`, which works with providers that do not return `preferred_username`.
...
- `SECRET_KEY` is generated inside the container and stored on a persistent volume. `utils_secret` is generated by default and should be kept stable for an existing deployment.
...
4. Choose attachment storage:
   - `local` is the default and stores attachments on a persistent volume attached to the Outline pod.
   - `s3` creates and uses a Sealos object storage bucket when object storage is available in your workspace.
...
When using Casdoor, create or edit a Casdoor application and add the same value to **Redirect URLs**. Then use the Casdoor application client ID and client secret for `oidc_client_id` and `oidc_client_secret`.

Typical Casdoor endpoints are:

| Outline input | What to enter for Casdoor |
|---------------|---------------------------|
| `oidc_auth_uri` | `https://<your-casdoor-domain>/login/oauth/authorize` |
...
Use the public HTTPS Casdoor domain for `oidc_auth_uri` because the browser is redirected there. The token and userinfo endpoints may use the in-cluster HTTP service address so the Outline pod can call Casdoor directly.

For example, if the Casdoor app service is `casdoor-gaklguki` in namespace `ns-admin`, use:
